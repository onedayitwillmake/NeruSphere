...
//	mFftDataRef = ci::audio::calculateFft( mBuffer->getChannelData( ci::audio::CHANNEL_FRONT_LEFT ), bandCount );
...
//			timeLine.push_back( ci::Vec2f( (float)i * scale + 10.0f,
//										  timeData[ i ] * ( windowHeight - 20.0f ) * 0.25f + ( windowHeight * 0.25f + 10.0f ) ) );
//			
//		}
//		
//		
//		float sum = 0.0f;
//		for ( int32_t i = 0; i < dataSize; ++i ) {
//			sum += ci::math<float>::pow( timeData[ i ], 2.0f );
//		}
//		mAverageVolume = ci::math<float>::sqrt( sum / (float)dataSize );
...
//	}
	...
	float scale = ( (float)AppInfo::getInstance().getWindowWidth() - 20.0f ) / bandCount;
	float logSize = ci::math<float>::log( bandCount );
	float sum = 0;
	
	
	ci::PolyLine<ci::Vec2f> freqLine;
	for( int i = 0; i < bandCount ; i++ ) {
		float amplitude = fftBuffer[i] / bandCount * ht;
		sum += amplitude;
		
		float x = (float)( ( ci::math<float>::log( (float)i ) / logSize ) * bandCount );;
		float y = amplitude;
	...
	mAverageVolume = sum/(bandCount/2);
	ci::gl::draw( freqLine );
